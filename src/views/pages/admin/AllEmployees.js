...
const columns = [
  ...
  {
    name: 'Day',
    selector: row => row.data.map((s)=>s.day),
    sortable:true
},
{
    name: 'Time',
    selector: row => row.data.map((s)=>s.time && moment(s.time, ["HH:mm"]).format("hh:mm a")),
    sortable:true
}
];
...
const AllEmployees = () => {
  ...
  const getSingleManager = async () => {
    ...
    try {
      ...
      result=result.filter(i => i.role=='employee')
      setemployees(result)
      setPending(false);
    }
    catch(e) {
            ...
            {/* <table className="table mb-5 table-striped">
              <thead>
                <tr>
                  ...
                  <th scope="col">Time</th><th></th><th></th><th></th><th></th>
                </tr>
              </thead>
